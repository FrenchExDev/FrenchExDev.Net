...
devAppHost.EnsureHostsSetup(dnsConfig);
devAppHost.EnsureSystemSetup(rootConfig, dnsConfig);
devAppHost.EnsureHostsSetup(dnsConfig);

var builder = DistributedApplication.CreateBuilder(args);

devAppHost.SetupBuilder(builder);
devAppHost.SetupProject(builder.AddProject<Projects.FrenchExDev_Net_CSharp_ProjectDependency5_Api>("api"), dnsConfig, "api", $"https://0.0.0.0:8443;{dnsConfig.GetUrl("api", 8443)}", 8443);
...
public interface IDevAppHost
{
    ...
    IDevAppHost SetupProject(IResourceBuilder<ProjectResource> resourceBuilder, DnsConfiguration dnsConfig, string name, string aspNetCoreUrls, int port);
}
...
public class DevAppHost : IDevAppHost
{
    ...
    public void SetupBuilder(IDistributedApplicationBuilder builder)
    {
        _builder = builder;
    }

    protected IDistributedApplicationBuilder BuilderOrDie() => _builder ?? throw new InvalidOperationException("Builder not initialized. Call SetupBuilder first.");

    public IDevAppHost SetupProject(IResourceBuilder<ProjectResource> resourceBuilder, DnsConfiguration dnsConfig, string name, string aspNetCoreUrls, int port)
    {
        resourceBuilder
            ...
            .WithEnvironment("CustomDomain__Fqdn", dnsConfig.GetUrl(name))
            .WithEnvironment("CustomDomain__Port", port.ToString())
            .WithUrl(dnsConfig.GetUrl(name, port));
