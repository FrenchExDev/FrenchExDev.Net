...
public class RsyncCommandBuilderTests : CommandBuilderTester<RsyncCommandBuilder, RsyncCommand>
{
    [Fact]
    public void Build_WithWorkingDirectory_Succeeds()
    {
        Valid(
            ...
            args => { args.ShouldBe(new[] { "rsync" }.ToList()); }
        );
    }
    ...
    public void Build_WithoutWorkingDirectory_FailsValidation()
    {
        Invalid(
            ...
            failures =>
            {
                failures.Keys.ShouldContain(nameof(RsyncCommand.WorkingDirectory));
                var f = failures[nameof(RsyncCommand.WorkingDirectory)].First();
