...
public class AsyncLambdaObjectValidator<TClass> : IAsyncObjectValidator<TClass>
{
    /// <summary>
    /// Reference to the function that performs the validation
    /// </summary>
    public Func<TClass, CancellationToken, Task<IEnumerable<IObjectValidation>>> ValidatorFunc { get; init; }
    ...
    /// objects of type <typeparamref name="TClass"/>. The returned collection of <see cref="IAbstractObjectValidator"/> objects
    ...
    public AsyncLambdaObjectValidator(Func<TClass, CancellationToken, Task<IEnumerable<IObjectValidation>>> validatorFunc)
    {
    ...
    /// cref="IEnumerable{T}"/> of <see cref="IAbstractObjectValidator"/> objects representing the validation results. The collection
    /// will be empty if no validation errors are found.</returns>
    public Task<IEnumerable<IObjectValidation>> ValidateAsync(TClass instance, CancellationToken cancellationToken = default)
    {
