...
public interface IObjectValidator<TClass> : IAbstractObjectValidator
{
    /// <summary>
    ...
    /// <returns>An <see cref="IEnumerable{T}"/> of <see cref="IObjectValidation"/> objects representing the validation results. The
    /// collection will be empty if the instance is valid.</returns>
    IEnumerable<IObjectValidation> Validate(TClass instance);
}
